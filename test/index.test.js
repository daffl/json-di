...
import loadAndProcess from '../src/';

const { load, process } = loadAndProcess;
...
describe('json-di', () => {
  ...
  it('`process` processes a JSON configuration', function(done) {
    ...
    process(loaded).then(data => {
      assert.deepEqual(data, {
        result: path.join('world', 'thing'),
        extended: 'world'
      });
      done();
    }).catch(done);
  ...
  it('`default` processes a JSON configuration runs a converter', function(done) {
    ...
    loadAndProcess({
      require: './loader/first.json'
    }, __filename, converter).then(data => {
      assert.deepEqual(data, {
        result: path.join('world', 'converted'),
        extended: 'world'
      });
      done();
    }).catch(done);
